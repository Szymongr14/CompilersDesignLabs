...
RE: RE RE %prec CONCAT {
  int first_set = RE_NULL($1) ? merge_sets(RE_FIRST($1), RE_FIRST($2)) : RE_FIRST($1);
  int last_set = RE_NULL($2) ? merge_sets(RE_LAST($1), RE_LAST($2)) : RE_LAST($2);

  $$ = createRE( RE_NULL($1) && RE_NULL($2), first_set, last_set, merge_follow_sets( merge_follow_sets(RE_FOLLOW($1), RE_FOLLOW($2)), set_product(RE_LAST($1), RE_FIRST($2))));
};
...
RE: RE '|' RE {
  $$ = createRE(
    RE_NULL($1) || RE_NULL($3),
    merge_sets(RE_FIRST($1), RE_FIRST($3)),
    merge_sets(RE_LAST($1), RE_LAST($3)),
    merge_follow_sets(RE_FOLLOW($1), RE_FOLLOW($3))
  );
};
